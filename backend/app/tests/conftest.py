...
async def professor_user(create_test_database):
    async with async_session() as session:
        result = await session.execute(
            select(User).where(User.email == professor_user_email)
        )
        user = result.scalars().first()
        if user is None:
            new_user = User(
...
async def generic_user(create_test_database):
    async with async_session() as session:
        result = await session.execute(
            select(User).where(User.email == generic_user_email)
        )
        user = result.scalars().first()
        if user is None:
            new_user = User(
...
async def homework(create_test_database):
    async with async_session() as session:
        result = await session.execute(
            select(Homework).where(Homework.id == test_homework_id)
        )
...
async def homeworkUser(homework, professor_user):
    async with async_session() as session:
        result = await session.execute(
            select(HomeworkUser).where(HomeworkUser.id == homework_user_test_id)
        )
