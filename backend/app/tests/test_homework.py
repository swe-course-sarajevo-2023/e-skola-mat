...
from app.models import Class, Homework, HomeworkStatus, User, taskUserHomework
...
async def test_get_homeworks_doesnt_exist(
    client: AsyncClient, professor_user_headers, session: AsyncSession
):
    response = await client.get(
        "/homeworks/get_homework_data/f4afd65a-c5b6-4ab6-a42b-81e1c4fff336",
        headers=professor_user_headers,
    )
    assert response.status_code == 204


async def test_add_homework(
    client: AsyncClient, professor_user_headers, session: AsyncSession
):
    new_homework_data = {
        "name": "Test Homework",
    ...
    response = await client.post(
        "/homeworks", headers=professor_user_headers, json=new_homework_data
    )
    assert response.status_code == 200


async def test_delete_homework(
    client: AsyncClient, professor_user_headers, homework, session: AsyncSession
):
    response = await client.delete(
        "/homeworks/" + homework.id, headers=professor_user_headers
    )
    assert response.status_code == 204
...
async def test_get_homework_user_details(
    client: AsyncClient, professor_user_headers, homeworkUser, session: AsyncSession
):
    response = await client.get(
        app.url_path_for(
            "get_homework_user_details",
            homework_user_id=homeworkUser.id,
            homework_id=homeworkUser.homework_id,
        ),
        headers=professor_user_headers,
    )
    assert response.status_code == 200
...
async def test_get_homework_user_details_403(
    client: AsyncClient, generic_user_headers, session: AsyncSession
):
    response = await client.get(
        app.url_path_for(
            "get_homework_user_details",
            homework_user_id="e7854775-67ae-43c4-a1c4-64d806569f22​​",
            homework_id="f7854224-67ae-43c4-a1c4-64d806569f22",
        ),
...
async def test_update_homework_status(
    client: AsyncClient, professor_user_headers, homework, session: AsyncSession
):
...
async def test_get_homework_data(
    ...
    response = await client.get(
        f"/homeworks/get_homework_data/{homework.id}", headers=professor_user_headers
    )
    assert response.status_code == 204
...
async def test_submit_comment(
    client: AsyncClient, task, professor_user_headers, session: AsyncSession
):
    response = await client.post(
        app.url_path_for("submit_comment"),
...
async def test_submit_comment_error(
    client: AsyncClient, professor_user_headers, session: AsyncSession
):
    response = await client.post(
        app.url_path_for("submit_comment"),
